...
class PolyvoiceObserver(BaseObserver):
    ...
    async def on_pipeline_started(self):
        if self._writer is None:
            self._writer = self.create_task(self._drain())  # task manager is set by setup()
    ...
    async def cleanup(self):
        try:
            ...
            if self._writer is not None:
                await self.cancel_task(self._writer)
            await super().cleanup()
    ...
    def _emit(self, t: _Turn, end_reason: str | None):
        rec = {
            "session_id": self._session_id, "turn": t.turn, "speculation": t.speculation, "discarded": t.discarded,
        ...
        self._queue.put_nowait(json.dumps(rec, default=_json_default, ensure_ascii=False))
